...
class ServerRegistry:
    ...
    def listUsers(self, uid):
        ...
        if uid != 0:
            ...
            if user is not None:
                return [{"id": user.id, "RSAPublicKey": user.description["RSAPublicKey"], "Certificate": user.description["Certificate"]}]
            return None

        userList = []
        for k in self.users.keys():
            newUser = {"id": k, "RSAPublicKey": self.users[k].description["RSAPublicKey"], "Certificate": self.users[k].description["Certificate"]}
    ...
    def newFile(self, basename, basename2):
        i = 1
        while True:
            path = basename+str(i)
            path2 = basename2+str(i)
    ...
    def sendMessage(self, src, dst, msg, receipt):
        ...
        try:
            ...
            nr = self.newFile(path, path2)
            self.saveOnFile(path + nr, json.dumps(msg))
    ...
    def readMsgFile(self, uid, msg):
        ...
        return json.loads(self.readFromFile(path))
    ...
    def storeReceipt(self, uid, msg, receipt, timestamp):
        ...
        path = self.userReceiptBox(os.path.join(m.group(1), "_%s_%s_%d" % (uid, m.group(2), timestamp)))
